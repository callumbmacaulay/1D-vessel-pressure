...
void bound_bif (double theta, double gamma, Tube *Arteries[], int parent, int D1, int D2, int WM, int VP)
{
    ...
    {
        double fvec[18];
        for (int row=0; row<18; row++) fvec[row]=0.0;
        // The residuals.
        ...
        //   fprintf(stdout,"at %d chi = %lf\n",iii,chi[iii]);
        //     fprintf(stdout,"at %d xb = %lf \n",iii,xb[iii]);
        // }
        // fprintf(stdout,"\n\n\n");
        ...
        //  // fvec[0]  = k1[0]  - xb[0] -
         //    theta*(qA2_term*sq(xb[2])/xb[11] + PV->Bh(N,xb[11],WM)) +
         //    gamma*(F(xb[2],xb[11],VP)+PV->B_RHSh(N,xb[11],WM));
        // fprintf(stdout,"%lf %lf %lf %lf %lf\n",k1[0],xb[0],PV->Bh(N,xb[11],WM),F(xb[2],xb[11],VP),PV->B_RHSh(N,xb[11],WM));
        // exit(1);
    ...
    if (j >=ntrial) {
        ...
        // fprintf(stdout,"\n\n\n");
        PV->Anew[N] = PV->Anew[N-1];
        PV->Qnew[N] = PV->Qnew[N-1];
        B1->Anew[0] = B1->Anew[1];
        B1->Qnew[0] = B1->Qnew[1];
        B2->Anew[0] = B2->Anew[1];
        B2->Qnew[0] = B2->Qnew[1] ;

        fprintf(stdout,"%lf %lf %lf %lf %lf %lf\n",PV->Anew[N],PV->Qnew[N],B1->Anew[0],B1->Qnew[0],B2->Anew[0],B2->Qnew[0]);
        // exit(1);
    }
}
