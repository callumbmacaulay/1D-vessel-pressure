...
int main(int argc, char *argv[])
{
    ...
    Rd1    = atof(argv[3]);
    CT1    = atof(argv[4]);
    p0     = atof(argv[5])*conv/rho/g/Lr;
    p_out  = atof(argv[6])*conv/rho/g/Lr;
    fileID = atoi(argv[7]);
...
// =========================NETWORK =================================================================================
   ...
   if (conn == NULL)
   {
    ...
    //Single vessel
    dimensions_matrix[0][0] = 50.00;
    dimensions_matrix[0][1] = 1.50;
    dimensions_matrix[0][2] = 1.50;

    terminal_vessels[0] = 0;
    Arteries[0] = new Tube( dimensions_matrix[0][0], dimensions_matrix[0][1], dimensions_matrix[0][2],
                                    daughter_ptr, number_of_points, 1,f1,f2,f3, Rp1,Rd1,CT1,0,0);
  ...
  finaltime = (period_counter+(cycles-1))*Period;
    while (tend <= finaltime)
    {
        ...
        for (int save_id=0; save_id<nbrves; save_id++)
         {
            for (int i=0; i<Arteries[save_id]->N; i++)
            {
                Arteries[ save_id] -> printAllt (art_ALL, tend, i, WALL_MODEL);
            }
        }
