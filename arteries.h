...
extern double   conv, rho, mu, nu, Lr, Lr2, Lr3, g, q, Fr2,
                Re, p0, p_out, pmean, tmst, Period, Fcst, CO, COm,
                Deltat, tau, vsmc_beta, vsmc_k, vsmc_kref,
            *fjac[24], xr, f, df;
...
public:
 ...
 // Prints P(x_fixed,t), A(x_fixed,t), F(x_fixed,t), or Q(x_fixed,t) for all
  ...
  double A_from_P (int i, double P, int WM);
      double A_from_Ph (int i, double P, int WM);


    // Defines Ph(x,A(x,t)).
  double Ph (int i, double A, int WM);
