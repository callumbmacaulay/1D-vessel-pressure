...
int    tmstps = 8192*4,                     // The number of timesteps per period.
...
const int WALL_MODEL = 1;// Define which wall model to use.
double p0, p_out;
double conv   = 1333.220,               // Conversion from mmHg to SI-units.
       ...
       Tper   = 0.9,//0.85,                   // Dimensional period [s].
       ...
       bound_thick = sqrt(Tper*nu/(2.0*M_PI))/Lr, // Boundary layer thickness
        ...
        // p0     = 4.0*conv/rho/g/Lr,      // Ensures a certain diastolic pressure.
        vsmc_k = 0.085, //Pewowaruk: VSMC activation
        vsmc_kref = 0.15,  //Pewowaruk: baseline VSMC activation
        vsmc_beta = 15.0, // Pewowaruk: beta term for VSMC
