...
{
  fprintf (fd, "%13.10f %15.10f\n", t*Lr3/q, (P(i,Anew[i],WM))*rho*g*Lr/conv);
}
...
{
    fflush(stdout);
    fprintf (fd, "%13.10f %13.10f %15.10f %15.10f %15.10f %17.10f\n", t*Lr3/q, i*h*Lr, (P(i,Anew[i],WM))*rho*g*Lr/conv, Qnew[i]*q, Anew[i]*Lr2, c(i, Anew[i],WM));// NEED TO REDIMENSIONALIZE WAVESPEED
}
...
void Tube :: printPxt (FILE *fd, double t, int offset, int WM)
{
  ...
  {
      fprintf (fd, "%13.10f %13.10f %15.10f %15.10f %15.10f %17.10f\n",
               t*Lr3/q, (i+offset)*h*Lr, (P(i,Anew[i],WM))*rho*g*Lr/conv, Qnew[i]*q, Anew[i]*Lr2, c(i, Anew[i],WM)); // modifeid to print more variables
  }
}
...
void Tube :: printPQ (FILE *fd, int i, int WM)
{
  fprintf (fd,"%15.10f %15.10f\n",
           Qnew[i]*q, (P(i,Anew[i],WM))*rho*g*Lr/conv);
}

void Tube :: printPA (FILE *fd, int i, int WM)
{
  fprintf (fd,"%15.10f %15.10f\n",
           (P(i,Anew[i],WM))*rho*g*Lr/conv, Anew[i]*Lr2);
}
...
double Tube :: P (int i, double A, int WM)
{
      double pold;
    if (WM==1){
        pold = fr[i]*(1.0-sqrt(A0[i]/A))+p0;
    }
    else if(WM==2) {
        pold = fr[i]*(sqrt(A/A0[i])-1.0)+p0;
    }
    else if (WM==3) {
        pold = p0*exp(fr[i]*(sqrt(A/A0[i])-1.0));
    }
     else {
            double p1 = exp(fr[i]*(sqrt(A/A0[i])-1.0));
            double p2 = (vsmc_k/vsmc_kref)*exp(vsmc_beta*((1.0/(1.0-vsmc_k))*sqrt(A/A0[i])-1.0));
            pold = p0*(p1+p2);
    }
   // fprintf(stdout,"P=%lf, strain=%lf\n",pold*rho*g*Lr/conv,sqrt(A/A0[i])-1.0);
    return pold;
}

// Pressure area relation
double Tube :: A_from_P (int i, double P, int WM)
{
      double A_val;
    if (WM==1){
         A_val = A0[i]/sq(1.0 - (P-p0)/fr[i]);
    }
    else if(WM==2) {
        A_val = A0[i]*sq(1.0+(P-p0)/fr[i]);
    }
    else if (WM==3) {
        error("arteries.C","not valid");
    }
     else {
            error("arteries.C","not valid");
    }
...
// Pressure area relation
double Tube :: A_from_Ph (int i, double P, int WM)
{
      double A_val;
    int ip1 = i+1;
    if (WM==1){
         A_val = A0h[i]/sq(1.0 - (P-p0)/frh[i]);
    }
    else if(WM==2) {
        A_val = A0h[i]*sq(1.0+(P-p0)/frh[i]);
    }
...
double Tube :: Ph (int i, double A, int WM)
{
      double pold;
    int ip1 = i; // Don't include half spatial step since this can go beyond the array indices
      if (WM==1){
          pold = frh[ip1]*(1.0-sqrt(A0h[ip1]/A))+p0;
      }
      else if(WM==2) {
          pold = frh[ip1]*(sqrt(A/A0h[ip1])-1.0)+p0;
      }
    else if (WM==3){
        pold = p0*exp(frh[ip1]*(sqrt(A/A0h[ip1])-1.0));
    }
    else {
         double p1 = exp(frh[ip1]*(sqrt(A/A0h[ip1])-1.0));
            double p2 = (vsmc_k/vsmc_kref)*exp(vsmc_beta*((1.0/(1.0-vsmc_k))*sqrt(A/A0h[ip1])-1.0));
            pold = p0*(p1+p2);
    }
   // fprintf(stdout,"frh[ip1]=%lf, A0h=%lf\n",frh[ip1],A0h[ip1]);
    return pold;
}
...
double Tube :: dPdA (int i, double A, int WM)
{
    double pold;
      if (WM==1){
        ...
        pold = 0.5*fr[i]*P(i,A,WM)/sqrt(A0[i]*A); //Derived
        // double denom = 2.0*A0[i]*(fr[i]+log(P(i,A,WM)/p0));
        // pold = sq(fr[i])*P(i,A,WM)/denom;//From Eck
        // pold = 2.0*A/(P(i,A,WM)*(fr[i]+log(P(i,A,WM)/p0)));
    }
    else {
        double term1 = p0/2.0/sqrt(A*A0[i]);
        double p1 = exp(fr[i]*(sqrt(A/A0[i])-1.0));
        double p2 = (vsmc_k/vsmc_kref)*exp(vsmc_beta*((1.0/(1.0-vsmc_k))*sqrt(A/A0[i])-1.0));
        pold = term1*(fr[i]*p1+vsmc_beta*p2/(1.0-vsmc_k));
    }
    return pold;
}
...
double Tube :: dPdx1(int i, double A, int WM)
{
    ...
    else { // No tapering for WM3 or WM4
        //dfrdr0[i]*p0*exp(sqrt(A/A0[i])-1.0
        pold = 0.0;//(- fr[i]*sqrt(M_PI*A)*P(i,A,WM)/A0[i])*dr0dx[i];
        // fprintf(stdout,"dpdx = %lf\n",pold);
    }
    return pold;
}
...
double Tube :: B (int i, double A, int WM)
{
    ...
    if (WM==1){
       ...
       pold = fr[i]*(sqrt(cu(A)/A0[i])-A0[i])/Fr2/3.0;  // Linear B
        ...
        // double term1 = p0/2.0/sqrt(A0[i])/sq(fr[i])/Fr2;
        // double term2 = 2.0*sqrt(A)*exp(fr[i]*(sqrt(A/A0[i])-1.0));
        // double term3 = fr[i]*(fr[i]*A - 2.0*sqrt(A*A0[i]))+2.0*A0[i];
        // pold = term1*term2*term3;

        // Second attempt
        double term1 = (2.0/sq(fr[i])) - (2.0/fr[i])*sqrt(A/A0[i]);
        double term2 = A0[i]*term1+A;
         pold = P(i,A,WM)*term2/Fr2;
        ...
        double p1_term1 = (2.0/sq(fr[i])) - (2.0/fr[i])*sqrt(A/A0[i]);
        double p1_term2 = A0[i]*p1_term1+A;
        double p1_B = p0*p1*p1_term2/Fr2;

        double p2_term1 = (1.0-vsmc_k)*sqrt(A/A0[i])/vsmc_beta;
        double p2_term2 = sq(1.0-vsmc_k)/sq(vsmc_beta);
        double p2_B = p0*p2*(vsmc_k/vsmc_kref)*(2.0*A0[i]*(p2_term1+p2_term2)+A);
...
double Tube :: Bh (int i, double A, int WM)
{
   int ip1 = i+1;
    ...
    if (WM==1){
        ...
        // double term1 = p0/2.0/sqrt(A0h[ip1])/sq(frh[ip1])/Fr2;
        // double term2 = 2.0*sqrt(A)*exp(frh[ip1]*(sqrt(A/A0h[ip1])-1.0));
        // double term3 = frh[ip1]*(frh[ip1]*A - 2.0*sqrt(A*A0h[ip1]))+2.0*A0h[ip1];
        // pold = term1*term2*term3;

        // Second attempt
        double term1 = (2.0/sq(frh[ip1])) - (2.0/frh[ip1])*sqrt(A/A0h[ip1]);
        double term2 = A0h[ip1]*term1+A;
         pold = Ph(ip1,A,WM)*term2/Fr2;
        // fprintf(stdout,"ph %lf, term1 %lf,m term2 %lf\n",Ph(ip1,A,WM),term1,term2);
    }
    else {
        double p1 = exp(frh[ip1]*(sqrt(A/A0h[ip1])-1.0));
        double p2 = (vsmc_k/vsmc_kref)*exp(vsmc_beta*((1.0/(1.0-vsmc_k))*sqrt(A/A0h[ip1])-1.0));

        double p1_term1 = (2.0/sq(frh[ip1])) - (2.0/frh[ip1])*sqrt(A/A0h[ip1]);
        double p1_term2 = A0h[ip1]*p1_term1+A;
        double p1_B = p0*p1*p1_term2/Fr2;

        double p2_term1 = (1.0-vsmc_k)*sqrt(A/A0h[ip1])/vsmc_beta;
        double p2_term2 = sq(1.0-vsmc_k)/sq(vsmc_beta);
        double p2_B = p0*p2*(vsmc_k/vsmc_kref)*(2.0*A0h[ip1]*(p2_term1+p2_term2)+A)/Fr2;
...
double Tube :: B_RHS (int i, double A, int WM)
{
    ...
    else { // MJC COME BACK HERE LATER; assume no tapering for now
        inteval = 0.0;
        pold = 0.0;
    }
...
double Tube :: dBdAh (int i, double A, int WM)
{
  int ip1      = i+1;
    ...
    if (WM==1){
        ...
        // double term1 = p0*frh[ip1]/2.0/sqrt(A0h[ip1])/Fr2;
        // pold = term1*sqrt(A)*exp(frh[ip1]*(sqrt(A/A0h[ip1])-1.0));
        pold = frh[ip1]*sqrt(A/A0h[ip1])*Ph(ip1,A,WM)/2.0/Fr2;
    }
    else {
        double term1 = p0*sqrt(A/A0h[ip1])/2.0/Fr2;
        double p1 = exp(frh[ip1]*(sqrt(A/A0h[ip1])-1.0));
        double p2 = (vsmc_k/vsmc_kref)*exp(vsmc_beta*((1.0/(1.0-vsmc_k))*sqrt(A/A0h[ip1])-1.0));
        pold = term1*(frh[ip1]*p1+vsmc_beta*p2/(1.0-vsmc_k));
    }
  return pold;
}
...
double Tube :: dB_RHSdAh (int i, double A, int WM)
{
   ...
   if (WM==1){
    ...
    else{ // MJC COME BACK; assume 0 with no tapering
            pold = 0.0;
    }
    return pold;
}
...
double Tube :: c (int i, double A, int WM) // The wave speed through aorta.
{
    ...
    // only needs to be nondimensionalized by sqrt(g*Lr)
      if (WM==1){
        ...
        cnst = 0.5*fr[i]*sqrt(A/A0[i])*P(i,A,WM)/Fr2; // Derived
           // cnst = 0.5*P(i,A,WM)*(fr[i]+log(P(i,A,WM)/p0))/Fr2;// From Eck 2017
    }
    else {
        double term1 = p0*sqrt(A/A0[i])/2.0/Fr2;
        double p1 = exp(fr[i]*(sqrt(A/A0[i])-1.0));
        double p2 = (vsmc_k/vsmc_kref)*exp(vsmc_beta*((1.0/(1.0-vsmc_k))*sqrt(A/A0[i])-1.0));
        cnst = term1*(fr[i]*p1+vsmc_beta*p2/(1.0-vsmc_k));
    }
    // fprintf(stdout,"c=%lf\n",sqrt(cnst));
    return sqrt (cnst);
}
...
{
    // if (t>Period){
    // For constant Area BC (using Lax-Wendroff)
   //  double A_BC = 5.90;
   //  double Pnew   = P(N, A_BC, WM);
   // double P05    = 0.5*(P(N,Anew[N],WM)-Pnew);
   // 
   // if (int(t/k) < 0)
        //   printf("t/k negative in bound_right\n");
        //
        // Anew[N] = A_from_P(N, Pnew, WM);

        // COnstant pressre
        double P_BC = p_out;
        double P05   =P(N-1,Aold[N],WM); //0.5*(P(N-1,Aold[N],WM)-p0);

        if (int(t/k) < 0)
            printf("t/k negative in bound_right\n");

        Anew[N] = A_from_P(N, P_BC, WM);

        double A05 = A_from_P(N, P05, WM);
        double Ahp05 = 2.0*A05 - Ah[N-1];
        double Qhp05 = R1h[N-1] + h*(Anew[N] - Aold[N])/k;
        double R2hp05 = Rvec(2,N,1,Qhp05,Ahp05,WM,VP);
        double S2hp05 = Svec(2,N,1,Qhp05,Ahp05,WM,VP);
        // Qnew[N] = Qold[N] - k*(R2h[N-1] - R2hm05)/h + k*(S2h[N-1] + S2hm05)/2.0;
            Qnew[N] = Qold[N] - k*(R2hp05-R2h[N-1])/h + k*(S2hp05+S2h[N-1])/2.0;

        // fprintf(stdout,"%lf %lf %lf %lf %lf %lf %lf %lf\n",Qhp05,Ahp05, R2h[N],R2hp05,S2h[N],S2hp05, A05, Ah[N]);
        // fprintf(stdout,"Qnew, Qold, Anew, Aold, Anew_n-1, Aold_n-1: %lf %lf %lf %lf %lf %lf\n",Qnew[N],Qold[N], Anew[N],Aold[N],Anew[N-1],Aold[N-1]);
        // exit(1);
    // }
    //     else
    //     {
    ...
    //         Anew[N] = Aold[N];
    //     }
    //     fprintf(stdout,"Qnew, P N, A05, and A N+1: %lf %lf %lf %lf\n",Qnew[N],Anew[N]);

    // For Windkessel
    // double qR, aR, cR, HpR, uR, cst, k1, k2;
    // int j = 1, ok = false, ch, ntrial = 5000;
    // 
    // qR = aR = cR = HpR = 0.0;
